...
export default function ShoppingCart() {
  ...
  const loginNotYet = () => {
    alert("請先登入");
    // 保存當前頁面的 URL 到 Cookies
    Cookies.set("redirectPath", window.location.href, { expires: 1 }); // 可設定有效期，例如 1 天
    window.location.href = "http://localhost:3000/login";
  };
  ...
  const processTransactions = async (cartData, userId) => {
    ...
    const promises = cartData.map((item) => {
      ...
      const data = {
        type: item.shoppingType === "class" ? "Class" : item.shoppingType, // 條件判斷
        detail: item.productID,
        userId: userId,
        price: item.price,
        status: "Pending Payment",
          ...
          {resultArray.map((item, index) => {
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    id="productImgForShoppingCart"
                    className="w-16 h-16 relative flex-shrink-0 mt-2 mb-2"
                  >
                    <img
