      ...
      try {
        ...
        const showcaseProductItems = data.payProductData
          ...
          .sort((a, b) => {
            ...
            const indexA = order.findIndex((keyword) =>
              a.name.includes(keyword)
            );
            const indexB = order.findIndex((keyword) =>
            ...
            const numA = a.name.match(/(\d+)/)
              ? parseInt(a.name.match(/(\d+)/)[1], 10)
              : 0;
            const numB = a.name.match(/(\d+)/)
              ? parseInt(b.name.match(/(\d+)/)[1], 10)
              : 0;
            return numA - numB;
          });
            ...
            >
              ...
              {!product.clickable && (
                <p className="disabled-info">
                  Valid until:{" "}
                  {new Date(product.ValidDate).toLocaleDateString()}
                </p>
              )}
            </div>
          ))}
          ...
          <table>
            ...
            <tbody>
              ...
              {renderCalculationRows(
                payProductArray.find((product) => product.id === activeBoxId)
              )}
              <tr>
                <td colSpan="3">
                  <span>
