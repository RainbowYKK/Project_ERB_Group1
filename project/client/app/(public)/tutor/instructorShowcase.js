    ...
    try {
      ...
      const danceClassResponse = fetch(`http://localhost:3030/danceclass/${id}`)
        ...
        .then((teacherDanceClass) => {
          ...
          // teacherDanceClass.tutorClass.sort(
          //   (a, b) => new Date(a.date) - new Date(b.date)
          // );

          // 獲取今日日期，並確保時間為當天的 00:00
          const today = new Date();
          today.setHours(0, 0, 0, 0); // 將時間設置為午夜，方便比較

          // 過濾出日期為今日或之後的課程，並按日期排序
          teacherDanceClass.tutorClass = teacherDanceClass.tutorClass
            .filter((item) => new Date(item.date) >= today) // 過濾條件：日期大於或等於今日
            .sort((a, b) => new Date(a.date) - new Date(b.date)); // 按日期升序排列

          // 輸出結果
          // console.log(
          //   "過濾並排序後的 tutorClass:",
          //   teacherDanceClass.tutorClass
          // );

          // 此時，teacherDanceClass.tutorClass 已經被排序
          // console.log("排序後的 tutorClass:", teacherDanceClass);

          teacherClassFromDB(teacherDanceClass);
        })
        // .then((sortedTutorClass) => {
        //   teacherClassFromDB(sortedTutorClass);
        // })
        .catch((error) => {
