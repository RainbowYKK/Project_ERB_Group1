...
export default function PendingPayment({ data }) {
  ...
  const handleDelete = async (transactionsObjectId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const deletedDataArray = htmlContentData.filter(
          (item) => item._id !== transactionsObjectId
        );
  ...
  const fetchData = async () => {
    ...
    let tempFinalHtmlContent = finalHtmlContent.filter(
      (item) => item.status === "Pending Payment"
    );
    console.log(
      "Pending Payment 的 Final HTML Content with Data:",
      tempFinalHtmlContent
    );
                  ...
                  <div key={index} className="text-gray-800">
                    ...
                    >
                      ...
                      {item.type === "class" || item.type === "Class" ?
                        ...
                        {item?.detailContent?.data?.[0] && (
                          <span className="text-sm text-gray-800 mr-3 ml-5">
                            {item.type === "class" || item.type === "Class"
                              ? `Class code : ${item.detailContent.data[0].code}`
                              : item.type === "room rental" ||
                                item.type === "Room rental"
                              ? `Room rental : ${item.detailContent.data[0].roomType}`
                              : `Product : ${
                      ...
                      HK${item.price}
                        ...
                        <span className="text-sm text-gray-800 mr-3 ml-5">
                          ...
                          item.type === "Room rental"
                            ? item.price *
                              item.detailContent?.data?.[0]?.timeRange?.length
                            : item.price}
                        </span>
