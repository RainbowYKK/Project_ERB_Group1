...
mongoose
  .connect(
    "mongodb+srv://demo:Ab123456@erb.3et5k.mongodb.net/project?retryWrites=true&w=majority&appName=ERB"
  )
  .then(async () => {
    await Member.syncIndexes(); // Syncs schema indexes with MongoDB
...
router.post("/update-password", Authorization, async (req, res, next) => {
  const { objectId, password, role } = req.body;
  let user;
  ...
  try {
    if (role === "staff") {
      user = await Staff.updateOne(
        { _id: objectId },
        { $set: { password: encryptPW } }
      );
    } else if (role === "teacher") {
      user = await Teacher.updateOne(
        { _id: objectId },
        { $set: { password: encryptPW } }
      );
    } else if (role === "member") {
